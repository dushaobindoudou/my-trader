...
import { type NavGroup as NavGroupProps, type NavItem } from './types'
...
function checkIsActive(pathname: string, item: NavItem | { url: string }, mainNav = false) {
  return (
    pathname === item.url ||
    pathname.split('?')[0] === item.url ||
    ('items' in item && !!item?.items?.filter((i) => i.url === pathname).length) ||
    (mainNav &&
      pathname.split('/')[1] !== '' &&
